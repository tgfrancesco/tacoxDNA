...
from libs.pdb import FROM_ANGSTROM_TO_OXDNA, Atom, Nucleotide


def print_usage():
    print("USAGE:", file=sys.stderr)
    print(
        "\t%s PDB_file direction [use 35 or 53 if the PDB file has the nucleotides listed in the 3' -> 5' or 5' -> 3' direction, respectively]"
        % sys.argv[0],
        file=sys.stderr,
    )
    ...
    try:
        ...
        for k in args:
            if k[0] == "-m" or k[0] == "--models-as-strands":
                print(
                    "## Different models will be interpreted as different strands",
                    file=sys.stderr,
                )
...
def main():
            ...
            if line.startswith("ATOM"):
                ...
                if old_chain != "":
                    if na.chain_id != old_chain and len(strand) != 0:
                        print(
                            "WARNING: a TER statement separating different strands (%s and %s) is missing"
                        ...
                        print(
                            "WARNING: a TER statement separates strands having the same chain id (%s)"
                            % na.chain_id,
                            file=sys.stderr,
                        )
                ...
                if na.alternate != "":
                    if na.alternate == "A" or na.alternate == "1":
                        print(
                            "Alternate location for atom '%s' of residue '%s' encountered, using the line marked with the '%s' character."
                            % (na.name, na.residue, na.alternate),
                            file=sys.stderr,
                        )
                ...
                if not models_as_strands:
                    ...
                    print(
                        "MODEL line detected: using the first MODEL encountered (%s)"
                        % (N_model),
                        file=sys.stderr,
                    )
    ...
    print(
        "Using a box of size %g in oxDNA units (twice as big as the PDB bounding box size)"
        % (L),
        file=sys.stderr,
    )
