...
class Base(object):
    ...
    def __init__(self, local_id, base_info):
        ...
        try:
            ...
            print(
                "WARNING: base n.%s has no associated type, setting it to '%s'"
                % (self.global_id, self.val),
                file=sys.stderr,
            )
...
class Strand(object):
    ...
    def __init__(self, strand_id, base_info_lst, local_id_strart=0):
        ...
        self.bases = [
            Base(local_id, base_info)
            for local_id, base_info in enumerate(base_info_lst, local_id_strart)
        ]
    ...
    def __str__(self):
        return "Strand %d: %s" % (self.strand_id, [str(b) for b in self.bases])
...
def get_base_config(base):
    ...
    if not (type(paring_base5) is NoBase):
        if opts["isDNA"]:
            a1_vector, a3_vector, cm_pos = neighbor3_cal_vector(
                backbone_A_to_backbone_B,
                backbone_A_to_posA3_neibor,
                backboneA_to_backbone_B5,
            )
        else:
            a1_vector, a3_vector, cm_pos = neighbor3_cal_vector_RNA(
                backbone_A_to_backbone_B,
                backbone_A_to_posA3_neibor,
                backboneA_to_backbone_B5,
            )
    ...
    # do we have a duplex across the 5' end
        ...
        if opts["isDNA"]:
            a1_vector, a3_vector, cm_pos = neighbor5_cal_vector(
                backbone_A_to_backbone_B,
                backbone_A_to_posA5_neibor,
                backboneA_to_backbone_B3,
            )
        else:
            a1_vector, a3_vector, cm_pos = neighbor5_cal_vector_RNA(
                backbone_A_to_backbone_B,
                backbone_A_to_posA5_neibor,
                backboneA_to_backbone_B3,
            )
        ...
        cm_pos = base_vector
        a3_vector = np.array([0.0, 0.0, 1.0])
        a1_vector = np.array([0.0, 1.0, 0.0])
...
def write_topology_file(strands, topology_file):
    ...
    for strand in strands:
        for base in strand.bases:
            top_lines.append(
                "%d %s %d %d"
                % (strand.strand_id, base.val, base.get_down_id(), base.get_up_id())
            )
...
def write_oxview_file(strands, file_name):
    box_size = 1
    out = {"systems": [{"id": 0, "strands": []}]}

    for s in strands:
        strand = {
            "id": s.strand_id,
            "end3": s.bases[0].local_id,
            "end5": s.bases[-1].local_id,
...
def print_usage():
    ...
    print(
        "\t[-t\--default-base=A|C|G|T|R|i (R = random, i = any integer)]",
        file=sys.stderr,
    )
    print("\t[-f\--print-force-file]", file=sys.stderr)
    print("\t[-o\--print-oxview]\n", file=sys.stderr)
    print(
        "\tThe defaults options are --molecule=DNA, --tiamat-version=2, --default-base=R",
        file=sys.stderr,
    )
    exit(1)
...
def parse_options():
    ...
    try:
        ...
        for k in args:
            if k[0] == "-m" or k[0] == "--molecule":
                ...
                if k_arg == "DNA":
                    ...
                    print(
                        "The argument of '%s' should be either 'DNA' or 'RNA' (got '%s' instead)"
                        % (k[0], k[1]),
                        file=sys.stderr,
                    )
                    exit(1)
                ...
                if db != "R" and db not in DNA_BASES:
                    try:
                        ...
                        print(
                            "Invalid default base value '%s'. The only accepted values are %s, R or an integer)"
                            % (k[1], ", ".join(DNA_BASES)),
                            file=sys.stderr,
                        )
                        exit(1)
        ...
        if tiamat_version == 1:
            ...
            print(
                "The argument of '%s' should be either '1' or '2' (got '%s' instead)"
                % (k[0], k[1]),
                file=sys.stderr,
            )
            exit(1)
