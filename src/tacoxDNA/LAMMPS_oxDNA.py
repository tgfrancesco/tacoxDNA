...
import numpy as np
from libs import base, reader_lammps_init
from libs.constants import inertia_in_lammps, mass_in_lammps, number_oxdna_to_lammps
...
def main():
    if len(sys.argv) < 2:
        ...
        print(
            "\t%s lammps_data_file [lammps_trajectory_file]" % sys.argv[0],
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    for i in range(N):
        ...
        if next_bond != -1 and next_bond != i + 1:
            if conf.strand[i] != conf.strand[next_bond]:
                print(
                    "Wrong bond arising between two different strands", file=sys.stderr
                )
            else:
            ...
            while line:
                ...
                if line.startswith("ITEM: NUMBER OF ATOMS"):
                    ...
                    if natoms != conf.natoms:
                        print(
                            "ERROR: A configuration stored in the trajectory file contains a number of nucleotides %d that differs from the %d in the datafile"
                            % (natoms, conf.natoms),
                            file=sys.stderr,
                        )
                        sys.exit(1)
