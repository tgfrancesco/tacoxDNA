...
import numpy as np
from libs import base, utils
from libs.pdb import FROM_OXDNA_TO_ANGSTROM, Atom, Nucleotide
        ...
        for k in args:
            if k[0] == "-H" or k[0] == "--hydrogens":
                ...
                if k_arg.lower() == "true":
                    ...
                    print(
                        "The argument of '%s' should be either 'true' or 'false' (got '%s' instead)"
                        % (k[0], k[1]),
                        file=sys.stderr,
                    )
                    exit(1)
...
def main():
    ...
    if rmsf_file:
        with open(rmsf_file) as f:
            try:
                ...
                print(
                    "Parsing error in RMSF file. Invalid Format: %s" % e,
                    file=sys.stderr,
                )
                exit(1)
            try:
                ...
                print(
                    "Parsing error in RMSF file. Conversion to float failed : %s" % e,
                    file=sys.stderr,
                )
                exit(1)
    else:
    ...
    for s_id, strand in enumerate(s._strands):
        ...
        if opts["one_file_per_strand"]:
            out.close()
            print(
                "## Wrote strand %d's data to '%s'" % (s_id + 1, out_name),
                file=sys.stderr,
            )
