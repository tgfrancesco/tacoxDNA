...
def print_usage():
    ...
    print(
        "\t[-c\--closed] [-o\--open] [-h\--help] [-d\--dsDNA] [-s\--ssDNA] [-n\--nicked] [-p\--supercoiling VALUE] [-w\--writhe VALUE] [-e\--seed VALUE] [-q\--sequence FILE]",
        file=sys.stderr,
    )
    exit(1)
...
def parse_options():
    ...
    try:
        ...
        for k in args:
            if k[0] == "-c" or k[0] == "--closed":
                opts.closed = True
            if k[0] == "-o" or k[0] == "--open":
                opts.closed = False
            if k[0] == "-h" or k[0] == "--help":
                print_usage()
            if k[0] == "-d" or k[0] == "--dsDNA":
                opts.double = True
            if k[0] == "-s" or k[0] == "--ssDNA":
                opts.double = False
            if k[0] == "-n" or k[0] == "--nicked":
                opts.nicked = True
            if k[0] == "-p" or k[0] == "--supercoiling":
                opts.supercoiling = float(k[1])
            if k[0] == "-w" or k[0] == "--writhe":
                opts.writhe = float(k[1])
            if k[0] == "-e" or k[0] == "--seed":
                opts.seed = int(k[1])
            if k[0] == "-q" or k[0] == "--sequence":
                opts.sequence_file = k[1]
...
def main():
    ...
    if opts.closed:
        print(
            "Total Linking Number (LK) %f, composed of:"
            % ((nbases / pitch) * (opts.supercoiling + 1) + opts.writhe),
            file=sys.stderr,
        )
        print(
            "1) Equilibrium number of DNA turns %f" % (nbases / pitch), file=sys.stderr
        )
        print(
            "2) Target writhe %f = Topological writhe %f + Turns imposed by supercoiling  %f "
            % (
                opts.writhe + (nbases / pitch) * opts.supercoiling,
                opts.writhe,
                (nbases / pitch) * opts.supercoiling,
            ),
            file=sys.stderr,
        )
    ...
    if opts.sequence_file == None:
        ...
        try:
            ...
            print(
                "The sequence file '%s' is unreadable" % opts.sequence_file,
                file=sys.stderr,
            )
            exit(1)
        ...
        if len(sequence) != nbases:
            print(
                "The length of the given sequence (%d) should be equal to the number of coordinates in the centerline file (%d)"
                % (len(sequence), nbases),
                file=sys.stderr,
            )
            exit(1)
