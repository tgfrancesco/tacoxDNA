...
def move_along_vector(
    ...
    move_distance = (
        float(length) * 0.4 / 2.0
    )  # 0.4 is the length of a base pair in oxDNA units, move half the helixlength down
    return [
        point[0] - move_distance * vector[0],
        point[1] - move_distance * vector[1],
...
def rpoly_to_oxDNA(opts):
    ...
    for n, i in enumerate(data):
        ...
        q = Quaternion(
            w=float(i[9]), x=float(i[6]), y=float(i[7]), z=float(i[8])
        )  # find the helix roation Info from file
        vec = q.rotate(np.array([0.0, 0.0, 1.0]))  # use it to figure out direction
        vec2 = q.rotate(
            [0.65, -0.76, 0.0]
        )  # ad hoc onversion between rpoly rotation and cadnano utils
        ...
        for (
            ...
            new_position
        ):  # go through every coordinate to find the largest coordinate to figure out box size
            if j > largest_size:
                largest_size = j
            else:
                pass
        ...
        new_strands = generator.generate_or_sq(
            bp=n_bp, start_pos=new_position, direction=vec, perp=vec2
        )
    ...
    ):  # iterate through staple strand connections and connect the previously generated fragments
