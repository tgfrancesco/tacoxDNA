...
def vhelix_rotation_origami_sq(direction, perp):
    R = utils.get_rotation_matrix(direction, np.pi * 15.0 / 180)
    return np.dot(R, perp)


def vhelix_rotation_origami_he(direction, perp):
    R = utils.get_rotation_matrix(direction, np.pi * 160.0 / 180)
    return np.dot(R, perp)


def insert_loop_skip(
    strands, start_pos, direction, perp, rot, helix_angles, vhelix, nodes, use_seq, seqs
):
    ...
    for i in range(len(nodes.begin)):
        ...
        if end_gs - begin_gs != 0:
            ...
            length_change.append(0)
            new_angle.append(
                sum(helix_angles) / len(helix_angles)
            )  # append an average angle
            # adjust beginning/end indices according to length change
            begin_gs += length_change_total
            end_gs += length_change_total + length_change[i]
            new_nodes.add_begin(begin_gs)
    ...
    for i in range(len(nodes.begin)):
        ...
        for j in vhelix.skip[begin_ltr : end_ltr + 1]:
            if j == 1:
                helix_angles_new = np.delete(
                    helix_angles_new, begin_gs - deleted + inserted
                )
                deleted_this_iteration += 1
        for j in vhelix.loop[begin_ltr : end_ltr + 1]:
            for _ in range(j):
                helix_angles_new = np.insert(
                    helix_angles_new, begin_gs - deleted + inserted, new_angle[i]
                )
                inserted_this_iteration += 1

    g = cu.StrandGenerator()
    new_strands = g.generate_or_sq(
        len(helix_angles_new) + 1,
        start_pos=start_pos,
        direction=direction,
        perp=perp,
        double=True,
...
def add_slice(
    ...
    if (
        vhelix.num % 2 + strand_type
    ) % 2 == 0:  # strand and even num or staple and odd num
    ...
    new_strands = insert_loop_skip(
        strands, pos, direction, perp, rot, helix_angles, vhelix, nodes, use_seq, seqs
    )
    current_system.add_strand(
        new_strands[strand_type].get_slice(begin_slice, end_slice), check_overlap=False
    )
    return current_system


def add_slice_nupack(
    vhelix, strand_number, begin_helix, end_helix, index_lookup, strand_type
):
    length_change = 0
    skips = 0
    loops = 0
    ...
    while nucleotide < iter_length:
        ...
        if vhelix.skip[vhelix_base] != 1:
            if (strand_type + vhelix.num % 2) % 2 == 0:
                ...
                add_nuc = [
                    nucleotide + x for x in range(vhelix.loop[vhelix_base] + 1)[::-1]
                ]
            if strand_type == 0:
...
def build_nodes(vh):
    ...
    for i in range(len(vh.scaf)):
        ...
        if not (
            (prev == prev_stap and (prev == "begin" or prev == "end"))
            or (next_ == next_stap and (next_ == "begin" or next_ == "end"))
        ):
...
def generate_vhelices_origami_sq(vhelix_direction, vhelix_perp, h):
    ...
    if h.num % 2 == 0:
        ...
        pos = np.array(
            [h.col * DIST_SQUARE, h.row * DIST_SQUARE, (h.len - 1) * base.BASE_BASE]
        )
        direction = -vhelix_direction
        perp = -vhelix_perp
...
def generate_vhelices_origami_he(vhelix_direction, vhelix_perp, h):
    ...
    for i in range(len(helix_angles)):
        ...
        if modi == 0:
            ...
            helix_angles[i] = 720.0 / 21 * (np.pi / 180.0)
    ...
    for i in range(len(helix_angles)):
        if i % 21 == 20:
            helix_angles[i] = 720.0 * np.pi / 180 - total_sum

    # make the virtual helices
    if h.num % 2 == 0:
        pos = np.array(
            [h.col * np.sqrt(3) * DIST_HEXAGONAL / 2, h.row * 3 * DIST_HEXAGONAL / 2, 0]
        )
        direction = vhelix_direction
        perp = vhelix_perp
        rot = 0.0
...
class vhelix(object):
    ...
    def add_square(self, toadd, which):
        if which == "stap":
            ...
            base.Logger.log(
                "Cannot add square that is not scaf or stap. Dying now",
...
def parse_jsonobject(jsonobject):
    ...
    try:
        ...
        print(
            "Caught an error while converting '" + jsonobject + "', aborting",
            file=sys.stderr,
        )
        sys.exit(1)
...
def parsingCli(source_file, sequence_filename):
    ...
    if sequence_filename:
        ...
        base.Logger.log(
            "Using sequence file '%s'" % sequence_filename, base.Logger.INFO
        )
            ...
            for x in re.sub(pattern, "", line):
                if x in ["R", "r"]:
                    ...
                    try:
                        ...
                        base.Logger.log(
                            "KeyError while converting base names to integer; check the sequence file",
                            base.Logger.CRITICAL,
                        )
                        sys.exit()
            sequences.append(seq)
...
def cadnano_oxdna(
    ...
    sequences = (
        randomSequenceGenerator(cadsys) if input_sequences is None else input_sequences
    )

    is_1strand_in_multiple_vhelixes_special_case = (
        len(sequences) == 1 and len(cadsys.vhelices) > 1
    )
    ...
    if is_1strand_in_multiple_vhelixes_special_case:
        base.Logger.log(
            "One line detected in the sequence file. Since the cadnano file contains more than 1 virtual helix, the sequence found will be used as we were dealing with a single-strand system",
            base.Logger.INFO,
        )
    ...
    if not side:
        side = cadsys.bbox()
        base.Logger.log(
            "Using default box size, a factor %s larger than the size of the cadnano system"
            % str(BOX_FACTOR),
            base.Logger.INFO,
        )
    vhelix_direction_initial = np.array([0.0, 0.0, 1.0])
    vhelix_perp_initial = np.array([1.0, 0.0, 0.0])
    if lattice_type == "sq":
    ...
    for h in cadsys.vhelices:
        ...
        if lattice_type == "sq":
            strands, helix_angles, pos, rot, vhelix_direction, vhelix_perp = (
                generate_vhelices_origami_sq(
                    vhelix_direction_initial, vhelix_perp_initial, h
                )
            )
        elif lattice_type == "he":
            strands, helix_angles, pos, rot, vhelix_direction, vhelix_perp = (
                generate_vhelices_origami_he(
                    vhelix_direction=vhelix_direction_initial,
                    vhelix_perp=vhelix_perp_initial,
                    h=h,
                )
            )
        else:
                ...
                if s.V_1 == -1 and s.b_0 == -1:
                    ...
                    if h.num % 2 == 1:
                        slice_sys = add_slice(
                            ...
                            helix_angles,
                            0,
                            not single_strand_system,
                            sequences,
                        )
                        vh_vb2nuc = add_slice_nupack(
                            h, strand_number, begin_helix, end_helix, vh_vb2nuc, 2
                        )
                else:
                    base.Logger.log("unexpected square array", base.Logger.WARNING)
        ...
        if slice_sys.N_strands == 0:
            base.Logger.log(
                "No scaffold strand found in virtual helix n. %d: staples-only virtual helices are not supported"
                % h.num,
                base.Logger.WARNING,
            )
            continue
            ...
            if s.V_0 == -1 and s.b_0 == -1:
                if s.V_1 == -1 and s.b_0 == -1:
                    ...
                    if h.num % 2 == 0:
                        ...
                        vh_vb2nuc = add_slice_nupack(
                            h, strand_number, begin_helix, end_helix, vh_vb2nuc, 3
                        )
                else:
                    base.Logger.log("unexpected square array", base.Logger.WARNING)
            elif s.V_0 == h.num and abs(s.b_0 - i) == 1:
    ...
    for a in range(2):
        ...
        for pair in join_list:
            ...
            backbone_backbone_dist = strand1._nucleotides[-1].distance(
                strand2._nucleotides[0], PBC=False
            )
            absolute_bb_dist = np.sqrt(
                np.dot(backbone_backbone_dist, backbone_backbone_dist)
            )
            if absolute_bb_dist > 1.0018 or absolute_bb_dist < 0.5525:
                base.Logger.log(
                    "the backbone-backbone distance across joints is %f: it will have to be relaxed with preliminary simulations"
                    % absolute_bb_dist,
                    base.Logger.WARNING,
                )
    ...
    if (
        ...
        and single_strand_system
        and len(final_sys._strands) > 1
    ):
        base.Logger.log(
    ...
    for strand in final_sys._strands:
        ...
        for nuc in reverse_nucs:
            rev_strand.add_nucleotide(
                base.Nucleotide(nuc.cm_pos, nuc._a1, -nuc._a3, nuc._base, nuc._btype)
            )
        if strand._circular:
    ...
    for vh, vb in list(vh_vb2nuc_final._scaf.keys()):
        ...
        for nucii in nuciis:
            nuc = (
                len(rev_sys._strands[strandii]._nucleotides)
                - 1
                - (nucii - nnucs_to_here[strandii])
                + nnucs_to_here[strandii]
            )
            ...
            for k in pos_to_id.keys():
                if k[2] == True and (k[0], k[1], False) in pos_to_id.keys():
                    for i in range(len(pos_to_id[k])):
                        pair_file.write(
                            "{} {}\n".format(
                                pos_to_id[k][i], pos_to_id[(k[0], k[1], False)][i]
                            )
                        )
    ...
    for i in range(len(cadsys.vhelices)):
        vhelix_pattern[cadsys.vhelices[i].num] = (
            cadsys.vhelices[i].row,
            cadsys.vhelices[i].col,
        )

    if rev_sys.N == 0:
        base.Logger.log(
            "The generated configuration is empty: this might be due to this conversion module not supporting virtual helices containing no scaffold strands.",
            base.Logger.CRITICAL,
        )
        exit(1)
    ...
    if print_virt2nuc:
        with open("virt2nuc", "wb") as fout:
            ...
            print(
                "## Wrote nucleotides' index conversion data to virt2nuc",
                file=sys.stderr,
            )
    ...
    if print_oxview:
        ...
        id_to_nucleotide = {
            n.index: n for s in rev_sys._strands for n in s._nucleotides
        }

        # Find the index of the scaffold strand (there really should be some
        # easier way of doing this)
        strands = [
            id_to_nucleotide[nucId].strand
            for nucIds in pos_to_id.values()
            for nucId in nucIds
        ]
        ...
        for s in rev_sys._strands:
            for n in s._nucleotides:
                if n.index in id_to_pos:
                    ...
                    if len(paired) > 1:
                        n.pair = id_to_nucleotide[
                            [idx for idx in paired if idx != n.index][0]
                        ]

                    # Get the staple strand nucleotide at this position
                    staple_ids = [
                        n
                        for n in pos_to_id[(vh, vb)]
                        if id_to_nucleotide[n].strand != scaffold_index
                    ]
                    if len(staple_ids) > 0:
                        staple_nuc = id_to_nucleotide[staple_ids[0]]
                        # One cluster per combination of helix and staple strand
                        n.cluster = cluster_ids.setdefault(
                            (vh, staple_nuc.strand), len(cluster_ids) + 1
                        )
    ...
    print(
        "## Wrote data to '%s' / '%s'" % (configuration_file, topology_file),
        file=sys.stderr,
    )
