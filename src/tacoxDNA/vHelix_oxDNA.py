...
def export_oxDNA(ma_file, box_size):
    ...
    for line in vHelixFile:
        if "createNode" in line:
            ...
            if "HelixBase" in line:
                ...
                if base_number > 1:
                    if (
                        float(base_translation[0]) == 0.0
                        and float(base_translation[1]) == 0.0
                        and float(base_translation[2]) == 0.0
                    ):
                        print("defaulting")
                    first_base_trans["|%s|%s" % (base_parent, base_name)] = (
                        base_translation
                    )
                ...
                try:
                    ...
                    first_base_parents["|%s|%s" % (base_parent, base_name)] = (
                        base_parent
                    )
                except:
                    pass
    ...
    for key in first_base_parents:
        if key.split("|")[2].strip("'").rstrip("'") in dups:
            ...
            base_parents[key.split("|")[2].strip("'").rstrip("'")] = first_base_parents[
                key
            ]
    ...
    while moves != 0:
        ...
        for line in strand_list:
            ...
            for other_line in strand_list:
                ...
                if (
                    check == other_check
                    and check2 == other_check2
                    and checkend == other_checkend
                    and (i != j)
                ):
                    del strand_list[i - 1]
    ...
    for name_line in base_names:
        if "forw" in name_line:
            ...
            if partner_base is not None:
                ...
                partner_parent_translation = np.array([0.0, 0.0, 0.0])

            perp_versor = np.array([-link_versor[1], link_versor[0], 0.0])
            unrotated_CoM_element = (
                own_translation + (0.8494 * link_versor) - (0.1883 * perp_versor)
            )
            new_forw_pos = unrotated_CoM_element - ((0.6 * 0.8518) * link_versor)
            new_backw_pos = unrotated_CoM_element + ((0.6 * 0.8518) * link_versor)
            rot_mat = GetMatrix(
                own_parent_rotation[0], own_parent_rotation[1], own_parent_rotation[2]
            )
            forw_CoM_element = (
                np.dot(rot_mat, unrotated_CoM_element) + own_parent_translation
            )

            rotated_forw_pos = np.dot(rot_mat, new_forw_pos)
            if partner_base is not None:
                back_rot_mat = GetMatrix(
                    partner_parent_rotation[0],
                    partner_parent_rotation[1],
                    partner_parent_rotation[2],
                )
                backw_CoM_element = (
                    np.dot(back_rot_mat, unrotated_CoM_element)
                    + partner_parent_translation
                )
            ...
            correct_backw_pos = (
                rotated_backw_pos + partner_parent_translation
            ) / 0.8518
            if (
                (correct_forw_pos[0] == correct_backw_pos[0])
                and (correct_forw_pos[1] == correct_backw_pos[1])
                and (correct_forw_pos[2] == correct_backw_pos[2])
            ):
                print("error in translation")
                print(partner_base, name_line, correct_forw_pos)
                sys.exit()
            a1 = (correct_backw_pos - correct_forw_pos) / np.linalg.norm(
                correct_backw_pos - correct_forw_pos
            )
            back_a1 = -a1
            ...
            if partner_base is not None:
                ...
                if (
                    (new_base_trans[name_line][0] == new_base_trans[partner_base][0])
                    and (
                        new_base_trans[name_line][1] == new_base_trans[partner_base][1]
                    )
                    and (
                ...
                if (
                    new_forw_pos[0] == 0
                    and new_forw_pos[1] == 0
                    and new_forw_pos[2] == 0
                ) or (
                    new_backw_pos[0] == 0
                    and new_backw_pos[1] == 0
                    and new_backw_pos[2] == 0
                ):
    ...
    for line in base_names:
        ...
        if line not in base_a1s.keys():
            ...
            rot_mat = GetMatrix(
                parent_rotation[0], parent_rotation[1], parent_rotation[2]
            )
            new_base_trans[line] = (
                np.dot(rot_mat, base_pos) + parent_translation
            ) / 0.8518
    ...
    for strand in strand_list:
        ...
        if (
            strand[0] == strand[-1]
        ):  # find out if the strand is circlar i.e scaffold and remove last base (duplicate with first base)
            circular = 1
        ...
        if strand[1] not in base_a3s.keys():
            strand_a3.append(
                (first_comp_pos - first_CoM)
                / np.linalg.norm(first_comp_pos - first_CoM)
            )
        else:
            strand_a3.append(base_a3s[strand[1]])

        if (
            first_comp_pos[0] == first_CoM[0]
            and first_comp_pos[1] == first_CoM[1]
            and first_comp_pos[2] == first_CoM[2]
        ):
        ...
        for line in base_numbers:
            ...
            try:
                ...
                print(index)
        for base, low, up in zip(
            base_letters, lower_neighbour_list, upper_neighbour_list
        ):
